...
import { pendingForWork } from './coordination/inbox';
 ...
 */
export const VIEWS = ['home', 'resumen', 'trabajo', 'team', 'evidencia', 'brief', 'funnel', 'context', 'memory', 'decisions', 'resultados'] as const;
...
export function App() {
  ...
  const [teamThreadMembers, setTeamThreadMembers] = useState<Record<string, string>>({});
  ...
  /**
   * B3.1/B3.5: QUIEN ESTA ABIERTO EN LA VISTA EQUIPO, por Trabajo.
   *
   * Es una seleccion distinta de la del chat a proposito: leer el hilo de Paid
   * Media no tiene por que cambiar la conversacion que la persona tiene abierta
   * al lado. `null` cae en el coordinador del run, y sin run en el primero;
   * esa resolucion vive en `TeamView`, que es quien la dibuja.
   */
  const selectThreadMember = (memberId: string) => { if (work) setTeamThreadMembers(prev => ({ ...prev, [work.id]: memberId })); };
  /** "Abrir chat" desde la vista Equipo: la conversacion de ESE miembro, en la columna de al lado. */
  const openMemberChat = (memberId: string) => { selectMember(memberId); setLayout('conversation'); };
  /** "Ver hilo" desde la pestana de un miembro del chat: su hilo, en la vista Equipo. */
  const openMemberThread = (memberId: string) => { selectThreadMember(memberId); setView('team'); };
  ...
  return <div className={'app-shell' + (railed ? ' railed' : '') + (focusChat ? ' conversation-focus' : '') + (dragging ? ' dragging' : '')} style={{ ['--agent-width' as string]: `${agentWidth}px` }}>
    ...
    <main className="workspace" aria-hidden={focusChat} inert={focusChat}>
      ...
      {view === 'team' && <TeamView work={work} team={team} roles={roles} mode={mode} busy={busy || startingChat} selectedMemberId={work ? teamThreadMembers[work.id] ?? null : null} onSelectMember={selectThreadMember} onOpenChat={openMemberChat} coordinationRun={work ? coordination.run : undefined} pending={coordination.pending} onPauseCoordination={coordination.pauseRun} onResumeCoordination={coordination.resumeRun} onCancelCoordination={coordination.cancelRun} coordinationLog={work ? coordination.log : undefined} coordinationMessages={work ? coordination.messages : undefined} coordinationAsks={work ? coordination.openAsks : undefined} coordinationHires={work ? coordination.hires : undefined} coordinationGates={work ? coordination.gates : undefined} coordinationSupport={work ? coordination.support : undefined} formatDate={date} coordinationAuthority={work ? coordination.authority : undefined} onSetCoordinationAuthority={changeCoordinationAuthority} coordinationBudget={work ? coordination.budget : undefined} onSetCoordinationBudget={coordination.setBudget} coordinatorGrant={work ? coordination.coordinatorGrant : undefined} />}
