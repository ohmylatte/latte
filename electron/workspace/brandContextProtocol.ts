...
import { LIMITS, hasControlChars } from '../services/validation';
...
export function parseBrandContextJson(raw: string): BrandContextProposalInput | null {
  ...
  if (keys.length === 0 || keys.some((k) => !BRAND_CONTEXT_INPUT_KEY_SET.has(k))) return null;
  ...
  if (hasControlChars(record.text) || hasControlChars(record.rationale)) return null;
  return {
    text: record.text,
