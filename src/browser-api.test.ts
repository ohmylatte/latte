...
describe('explicit browser preview', () => {
  ...
  it('does not pretend to run an agent or memory server', async () => {
    ...
    // diciendo que ninguno está disponible. Son los tres de siempre más Grok y Hermes.
    const runtimes = await api.runtimeStatus();
    expect(runtimes.map(r => r.provider).sort()).toEqual(['claude', 'codex', 'grok', 'hermes', 'opencode']);
...
describe('marketing preview parity',()=>{
 ...
 it('tracks derived docs and acknowledges separately from approval',async()=>{const [base]=await api.listDocuments('demo-work');const derived=await api.createDocument('demo-work','copy','Anuncio',base.id);await api.updateDocument(derived.document.id,{status:'review'});await api.saveDocument(base.id,'changed', (await api.readDocument(base.id)).fingerprint);expect((await api.documentState(derived.document.id)).baseOutdated).toBe(true);expect((await api.acknowledgeBase(derived.document.id)).status).toBe('review');expect((await api.documentState(derived.document.id)).baseOutdated).toBe(false);});
  ...
  it('keeps the coordinator grant per work, so choosing who coordinates can be tried in the preview', async () => {
    expect(await api.getCoordinatorGrant('w1')).toBeNull();
    expect(await api.setCoordinatorGrant('w1', 'mem_a')).toBe('mem_a');
    expect(await api.getCoordinatorGrant('w1')).toBe('mem_a');
    expect(await api.getCoordinatorGrant('w2')).toBeNull();
    expect(await api.setCoordinatorGrant('w1', null)).toBeNull();
    expect(await api.getCoordinatorGrant('w1')).toBeNull();
  });
  it('mirrors the out-of-scope toggle: mark, round-trip, unmark and invalid stage',async()=>{const b=await api.createBrand('Marca');const w=await api.createWork(b.id,'Uno');expect((await api.listWorks(b.id))[0].outOfScopeStages).toEqual([]);expect((await api.toggleOutOfScopeStage(w.id,'conversion')).outOfScopeStages).toEqual(['conversion']);expect((await api.listWorks(b.id))[0].outOfScopeStages).toEqual(['conversion']);expect((await api.toggleOutOfScopeStage(w.id,'conversion')).outOfScopeStages).toEqual([]);await expect(api.toggleOutOfScopeStage(w.id,'propuesta' as never)).rejects.toThrow('Etapa inválida');await expect(api.toggleOutOfScopeStage('wrk_nope','discovery')).rejects.toThrow('Trabajo no encontrado');});
});
